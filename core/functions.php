<?php
	...
	function subirArchivo ($arr_file_desc, $destino=null, $name=null){
        ...
        if (file_exists($arr_file_desc['tmp_name'])){
           ...
           header('Content-type: application/json');
			...
			//objeto json que devolver� la respuesta
			$jsondata = array();
		    $jsondata['error']	= true;
        ...
        //si es una imagen, creo una m�s peque�a para agilizar la carga con thumbnails
    ...
    /**
     * Compara 2 fechas y devuelve la diferencia de d�as
	...
	/**
	 * Comprime un archivo con los m�todos de php
	...
	/**
	 * Obtiene el thumbnail de una foto subida con nuestro m�todo de subida de archivos
	...
	/**
	 * Encripta un string (an�logo al m�todo decrypt)
    ...
    /**
     * desencripta un string encriptado por el m�todo encrypt
     * @param unknown_type $text
     */
    ...
    /**
     * M�todo que pasa un valor a float, si tiene coma lo cambia por punto y si no tiene decimales le pone el .00
	...
	function to_float ($valor){
		if($valor==""){
		    ...
		    // si tiene punto y est� a la izquierda de la coma, eliminamos el punto porque es de miles
		    if($punto>-1){
		        // 22.000,00
		        if($punto < $coma){
	...
	/**
	 * M�todo que pone la coma a los decimales
	 * @param unknown_type $valor
	 */
	...
	/**
	 * Convierte un array en un string (método análogo a string_to_array)
	 * @param array $array
	 */
	...
	/**
	 * Devuelve el tama�o de un archivo en kb
	...
	/**
	 * M�todo que convierte un xml en un array
	...
	/**
	 * El m�todo add filter prepara los filtros que despu�s se compondr�n
	 ...
	 * llevar la concatenaci�n or para que funcione de manera correcta
	 */
    ...
    /**
     * M�todo que prepara los filtros para una query gen�rica
     * @param array $filters este array se prepara con el m�todo
     * add_filter
     */
	function prepare_filters($filters){
		...
		$filter_query	= "WHERE ( ";
       	...
       	// primero cuento los or que hay para que no me printe un "or (" al llegar al �ltimo or que haya.
        ...
        foreach($filters as $key=>$value ){
        	if(strtolower($value['concatenation'])=="or"){
        		...
        		//aqui solo pinto or si no es el �ltimo
	...
	/**
	 * M�todo Para la aplicaci�n de seguridad
	...
	/**
	 * M�todo que determina si un string es parte de un
	 * c�digo html o no.
	 * @param string $str
	 * @param bool $count si quieres que te diga d�nde empezar�a
	 */
	...
	/**
	 * M�todo que sirve como primerva barrera de limpieza
	 * para la prevenci�n de sql injection
	 * @param string $data
	 */
	function prevent_sqlinjection ($data){
		return addslashes(limpiarCadena(preg_replace('/[^·ÈÌÛ˙¡…Õ”⁄Ò—a-zA-Z0-9-_,;@.\s\n\r]/i', '', $data)));
	}
	
	/**
	 * M�todo asociado a prevent_sqlinjection
	...
	/**
	 * M�todo que hace una expulsi�n de seguridad del site
	 * borra sesi�n, elimina todas las variables de entorno y redirige a la home
    ...
    /**
     * Devuelve quien ha llamado a la funci�pn
	 ...
	 * va asociado el m�todo show_query
	 * @param array $data
	 */
	 ...
	 * de manera friendly, va asociado al m�todo show_query
	 * @param array $data
	 */
	...
	/**
	 * Traduce las tags de un xml. En xml no se pueden utilizar los
	 * símbolos "<" y ">", por eso se cambian por "#" y "-#" respectivamente.
	 * @param string $string cadena a interpretar.
	 */
	function traducirStringXml ($string) {
